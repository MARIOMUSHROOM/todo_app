...
class _DonePageState extends State<DonePage>
  ...
  DoneBloc? blocController;
  @override
  void initState() {
    blocController = BlocProvider.of<DoneBloc>(context);
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<DoneBloc, DoneState>(builder: (context, state) {
      ...
      if (state is DoneInitial) {
        BlocProvider.of<DoneBloc>(context).add(Init());
      } else if (state is DoneLoading) {
        return const ConnectionLoadingWidget();
      } else if (state is DoneLoadFailue) {
        return const ConnectionErrorWidget();
      } else if (state is DoneLoaded) {
          ...
          child: Slidable(
            ...
            endActionPane: ActionPane(
              ...
              children: [
                SlidableAction(
                  onPressed: (context) {
                    BlocProvider.of<DoneBloc>(context).add(Remove(only.id!));
                  },
